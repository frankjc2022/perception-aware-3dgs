...
from utils.score_utils import compute_gaussian_score_with_perception
from utils.unseen_camera_loss_utils import depth_smoothness_loss, total_variation_loss
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, websockets, score_coefficients, args):
    ...
    random_viewpoint_stack = scene.getRandomCameras().copy()
    random_viewpoint_indices = list(range(len(random_viewpoint_stack)))
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if args.unseen_loss:
            ...
            if not random_viewpoint_stack:
                random_viewpoint_stack = scene.getRandomCameras().copy()
                random_viewpoint_indices = list(range(len(random_viewpoint_stack)))
            random_rand_idx = randint(0, len(random_viewpoint_indices) - 1)
            random_viewpoint_cam = random_viewpoint_stack.pop(random_rand_idx)
            _ = random_viewpoint_indices.pop(random_rand_idx)

            with torch.no_grad():

                random_render = render(random_viewpoint_cam, gaussians, pipe, bg, 1.0)

                random_image = random_render["render"]
                random_depth = random_render["depth"]

                loss_smooth_total = depth_smoothness_loss(random_image, random_depth)
                loss_tv_total = total_variation_loss(random_image)

                lambda_smooth = 0.7
                lambda_tv = 0.3

                total_unseen_loss = lambda_smooth * loss_smooth_total + lambda_tv * loss_tv_total
        ...
        if not args.unseen_loss:
            loss = (1.0 - opt.lambda_dssim) * Ll1 + opt.lambda_dssim * (1.0 - ssim_value)
        else:
            lambda_unseen = 0.2
            loss = (1.0 - opt.lambda_dssim - lambda_unseen) * Ll1 + opt.lambda_dssim * (1.0 - ssim_value) + lambda_unseen * total_unseen_loss
        ...
        with torch.no_grad():
            ...
            if iteration < opt.densify_until_iter:
                ...
                if iteration > opt.densify_from_iter and iteration % opt.densification_interval == 0:
                    ...
                    if not args.combine_scores:
                        if args.segment_mask or args.depth_map or args.edge_mask or args.saliency_map:
                            gaussian_importance = compute_gaussian_score_with_perception(scene, camlist, edge_losses, gaussians, pipe, bg, score_coefficients, opt)
                        else:
                            gaussian_importance = compute_gaussian_score(scene, camlist, edge_losses, gaussians, pipe, bg, score_coefficients, opt)
                    else:
                        gaussian_importance_perception = compute_gaussian_score_with_perception(scene, camlist, edge_losses, gaussians, pipe, bg, score_coefficients, opt)
                        gaussian_importance_taming = compute_gaussian_score(scene, camlist, edge_losses, gaussians, pipe, bg, score_coefficients, opt)
                        ...
                        # taming_score = normalize(gaussian_importance_taming)
                        # perception_score = normalize(gaussian_importance_perception)
                        taming_score = gaussian_importance_taming
                        perception_score = gaussian_importance_perception

                        alpha = 0.7  # 0.7 structure score
                        beta = 0.3  # 0.3 perception score
                        gaussian_importance = alpha * taming_score + beta * perception_score
